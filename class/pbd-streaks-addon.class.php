    ...
    public function pbd_streaks_shortcode_callback( $atts = array() ) {
        ...
        foreach ( $reports as $key => $report ) {
            if ( $key == 0 ) {
                ...
                $next_date = date('Y-m-d', strtotime('+1 day', strtotime( $events[ $external_key - 1 ]['end'] )));

                if ( $next_date ==  $report['date'] ) {
                    $events[ $external_key - 1 ]['end'] = $report['date'];
                    $events[ $external_key - 1 ]['js_end'] = date('Y-m-d', strtotime('+1 day', strtotime( $report['date'] )));
                } else {
                    array_push( $events, [
                        'start' => $report['date'],
                        'end' => $report['date'],
        ...
        // only enqueue styles and scripts if needed
        wp_enqueue_style( 'pbd-sa-style' );
        wp_enqueue_style ( 'pbd-sa-fullcalendar-css', PBD_SA_URL . '/assets/fullcalendar/main.css');
        wp_enqueue_script( 'pbd-sa-fullcalendar-js', PBD_SA_URL . '/assets/fullcalendar/main.js');
